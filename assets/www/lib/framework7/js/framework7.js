    ...
    window.Framework7 = function (params) {
        ...
        ************   Ajax submit for forms   ************
        ...
        $(document).on('submit change', 'form.ajax-submit, form.ajax-submit-onchange', function (e) {
            ...
            var data;
            if(form.attr('id')=='profile_form'||form.attr('id')=='profile_edit_form'){
              if (method === 'POST') data = new FormData(form[0]);
              else data = $.serializeObject(app.formToJSON(form[0]));
              if(form.attr('id')=='profile_edit_form'){
                if( document.getElementById("profile_edit_form_ProfileImage").files[0]&&document.getElementById("profile_edit_form_ProfileImage").files[0].name){
                  console.log('profilesss');
                  data.delete('Image');
                }else{
                  data.delete('ProfileImage');
                  console.log('profile');
                }
              }

            }else{
              if (method === 'POST') data = app.formToJSON(form[0]);
              else data = $.serializeObject(app.formToJSON(form[0]));
